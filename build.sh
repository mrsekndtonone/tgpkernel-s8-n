...
if [ $S8OREO == "1" ]; then
	cp -rf $RDIR/.build/ramdisk-s8oreo/* $WORKDIR/ramdisk 
fi
}
...
dreamlte)
	...
	if [ $N8PORT == "1" ]; then
		sed -i -- 's/ro.sf.lcd_density=420/ro.sf.lcd_density=480/g' $WORKDIR/ramdisk/ramdisk/default.prop
		sed -i -- 's/ro.sf.init.lcd_density=560/ro.sf.init.lcd_density=640/g' $WORKDIR/ramdisk/ramdisk/default.prop
	fi
	cd $WORKDIR/ramdisk
	./repackimg.sh
	;;
